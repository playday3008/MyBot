...
Func InitBlueStacksX($bCheckOnly = False, $bAdjustResolution = False, $bLegacyMode = False)
	...
	SetBlueStacksRegPath()
	; more recent BlueStacks 2 version install VirtualBox based "plus" mode by default
	Local $plusMode = RegRead($g_sHKLM & $__BlueStacks_RegPath, "Engine") = "plus" And $bLegacyMode = False
	...
	$__BlueStacks_Version = RegRead($g_sHKLM & $__BlueStacks_RegPath, "Version")
	$__BlueStacks_Path = RegRead($g_sHKLM & $__BlueStacks_RegPath, "InstallDir")
	If @error <> 0 Then
	  If $__BlueStacks_HyperV Then
		  $__BlueStacks_Path = @ProgramFilesDir & "\BlueStacks_bgp64_hyperv\"
	  Else
	...
	If Not $bCheckOnly Then
		...
		Local $BootParameter = RegRead($g_sHKLM & $__BlueStacks_RegPath & "Guests\" & $g_sAndroidInstance & "\", "BootParameters")
		Local $OEMFeatures
...
Func ConfigureSharedFolderBlueStacksX($iMode = 0, $bSetLog = Default)
	...
	Switch $iMode
		Case 0 ; check that shared folder is configured in VM
			For $i = 0 To 5
				If RegRead($g_sHKLM & $__BlueStacks_RegPath & "Guests\" & $g_sAndroidInstance & "\SharedFolder\" & $i & "\", "Name") = "BstSharedFolder" Then
					$bResult = True
					$g_bAndroidSharedFolderAvailable = True
					$g_sAndroidPicturesPath = "/storage/sdcard/windows/BstSharedFolder/"
					$g_sAndroidPicturesHostPath = RegRead($g_sHKLM & $__BlueStacks_RegPath & "Guests\" & $g_sAndroidInstance & "\SharedFolder\" & $i & "\", "Path")
					ExitLoop
				EndIf
			Next
...
Func InitBlueStacks2($bCheckOnly = False)
	...
	If $bInstalled And Not $bCheckOnly Then
		...
		; read ADB port
		Local $BstAdbPort = RegRead($g_sHKLM & $__BlueStacks_RegPath & "Guests\" & $g_sAndroidInstance & "\Config\", "BstAdbPort")
		If $BstAdbPort Then
...
Func GetBlueStacks2BackgroundMode()
	...
	Local $GlRenderMode = RegRead($g_sHKLM & $__BlueStacks_RegPath & "Guests\" & $g_sAndroidInstance & "\Config\", "GlRenderMode")
	Switch $GlRenderMode
		Case 4
			; DirectX
			Return $g_iAndroidBackgroundModeDirectX
...
Func CheckScreenBlueStacksX($bSetLog = True)
	Local $REGISTRY_KEY_DIRECTORY = $g_sHKLM & $__BlueStacks_RegPath & "Guests\" & $g_sAndroidInstance & "\FrameBuffer\0"
	Local $aValues[5][2] = [ _
			["FullScreen", 0], _
...
Func SetScreenBlueStacks()
	...
	$REGISTRY_KEY_DIRECTORY = $g_sHKLM & $__BlueStacks_RegPath & "Guests\" & $g_sAndroidInstance
...
Func SetScreenBlueStacks2()
	...
	$REGISTRY_KEY_DIRECTORY = $g_sHKLM & $__BlueStacks_RegPath & "Guests\" & $g_sAndroidInstance & "\Config"
...
Func CloseUnsupportedBlueStacksX($bClose = True)
	Local $WinTitleMatchMode = Opt("WinTitleMatchMode", -3) ; in recent 2.3.x can be also "BlueStacks App Player"
	Local $sPartnerExePath = RegRead($g_sHKLM & $__BlueStacks_RegPath & "Config\", "PartnerExePath")
